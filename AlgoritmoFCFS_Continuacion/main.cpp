...
int main(){
    ...
    Id = 0;
    int NumProcs, i, j, interrupcion, EspaciosLL, EspaciosVas, ProcTer, ProcR, TiempG, NivelBCP, TiempP;
    float ResulProc;
    srand(time(NULL));
    ...
    do{
        ...
        if(kbhit() != 0){
            ...
            case 6:
                ...
                //Ciclo para imprimir la informacion de los procesos
                for(j = 0; j < NumProcs; j++){
                    ...
                    if(ObjProc.RegresaID() == 1){
                        ...
                        cout << left << setw(16) << "x";
                        //cout << left << setw(16)<< (TiempG - ObjProc.RegresaTiempoLl()) - ObjProc.RegresaTiempoTrans();
                    } else{
                        cout << left << setw(16)<< (TiempG - ObjProc.RegresaTiempoLl()) - ObjProc.RegresaTiempoTrans();
                    }
                    ...
                    //Caso inicial para los procesos Nuevos
                    if (AuxiliarBCP->RegresaLiga() != NULL){
                        AuxiliarBCP = AuxiliarBCP->RegresaLiga();
                    } else{
                        NivelBCP++;
                        switch(NivelBCP){
                        case 1:
                            //Caso para el proceso en Ejecucion y en lista de Listos
                            if (ListaListos.RegresaPrimero() != NULL){
                            ...
                            break;
                        case 2:
                            //Caso para bloqueados
                            if (ListaBloqueados.RegresaPrimero() != NULL){
                                AuxiliarBCP =  ListaBloqueados.RegresaPrimero();
                            } else {
                                if (ListaTerminados.RegresaPrimero() != NULL){
                                    AuxiliarBCP = ListaTerminados.RegresaPrimero();
                                }
                            }
                            break;
                        case 3:
                            //Caso para los terminados
                            if (ListaTerminados.RegresaPrimero() != NULL){
                                AuxiliarBCP = ListaTerminados.RegresaPrimero();
                            }
                            break;
